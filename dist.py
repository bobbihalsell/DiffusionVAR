...
def initialize(fork=False, backend='nccl', gpu_id_if_not_distibuted=0, timeout=30):
    ...
    try:
        tdist.init_process_group(backend=backend, timeout=datetime.timedelta(seconds=timeout*60), device_id=local_rank)
        global __rank, __local_rank, __world_size
        ...
        print(f'[dist initialize] Failed to initialize distributed training: {e}', file=sys.stderr)
        __device = torch.empty(1).cuda().device
        __initialized = False


def init_for_testing():
    """Initialize distributed system for testing purposes."""
