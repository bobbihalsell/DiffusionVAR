...
vis_args = {
    ...
    'n_images': 8,
    'max_res': 128,
    'data_path': '/vol/bitbucket/blh124/project/dvar/imagenet_subset10',  # UPDATE THIS TO YOUR MAIN DATASET PATH
    'save_path': 'visualisations/vqvae',
    # Toggle options
    'analyze_distribution': False,  # Set to True if you want distribution plots
    'single_image_only': True,     # Set to False if you want multiple images
    'max_dataset_size': 10,        # Only load this many images from dataset
}
...
class VAEVisualiser:
    ...
    def build_vqvae_batch(self):
        ...
        # Limit dataset size to avoid loading entire dataset
        if len(dataset) > self.args['max_dataset_size']:
            # Use a subset of the dataset
            indices = list(range(self.args['max_dataset_size']))
            dataset = Subset(dataset, indices)
            print(f'[DataLoader] Limited dataset to {len(dataset)} images (from {self.args["max_dataset_size"]} requested)')
        else:
            print(f'[DataLoader] Using all {len(dataset)} images (less than max_dataset_size={self.args["max_dataset_size"]})')
        
        # Adjust batch size based on whether we need distribution analysis
        batch_size = 1 if self.args['single_image_only'] else self.args['n_images']
        ...
        print(f'[DataLoader] Created dataloader with batch_size={batch_size} from {self.args["data_path"]}')
    ...
    def analyze_single_image(self, image):
        ...
        with torch.no_grad():
            ...
            # Get progressive residual analysis
            residual_analysis = self._analyze_progressive_residuals(image[:1])
            ...
            self.plot_progressive_residuals(image[0], residual_analysis)
    ...
    def plot_tokens_only(self, original_image, ms_idx_Bl):
        ...
        for i, idx_Bl in enumerate(ms_idx_Bl):
            pn = self.args['patch_nums'][i]
            tokens = idx_Bl[0].cpu().numpy()
            unique_tokens = len(np.unique(tokens))
            total_tokens = len(tokens)
            print(f"Scale {i} ({pn}×{pn}): {total_tokens} total tokens, {unique_tokens} unique tokens ({unique_tokens/total_tokens:.2%} diversity)")
    ...
    def _analyze_progressive_residuals(self, image):
        """
        Analyze the progressive residual quantization using actual VQVAE reconstructions
        """
        # Get the actual tokens from the image
        ms_idx_Bl = self.vae.img_to_idxBl(image)
        
        # Track states at each scale
        residuals = []  # What's left to quantize (top row)
        contributions = []  # What this scale contributed (middle row) 
        accumulated = []  # Accumulated reconstruction so far (bottom row)
        
        # Generate progressive reconstructions using actual tokens
        for i in range(len(ms_idx_Bl)):
            # Bottom row: Accumulated reconstruction up to scale i
            # Use tokens from scale 0 to scale i
            tokens_up_to_i = ms_idx_Bl[:i+1]
            accumulated_img = self.vae.idxBl_to_img(tokens_up_to_i, same_shape=True, last_one=True)[0]
            accumulated.append(accumulated_img)
            
            # Middle row: Contribution of scale i 
            if i == 0:
                # First scale contributes everything it reconstructs
                contribution_img = accumulated_img.clone()
            else:
                # Contribution = current accumulated - previous accumulated
                prev_accumulated = accumulated[i-1]
                contribution_img = accumulated_img - prev_accumulated
                # Clamp to valid image range after subtraction
                contribution_img = contribution_img.clamp(-1, 1)
            contributions.append(contribution_img)
            
            # Top row: Residual (what's left to quantize)
            if i == 0:
                # Before any quantization, residual is the original image
                residual_img = image.clone()
            else:
                # Residual = original - what we've reconstructed so far with previous scales
                prev_accumulated = accumulated[i-1] 
                residual_img = image - prev_accumulated
                # Clamp to valid image range after subtraction
                residual_img = residual_img.clamp(-1, 1)
            residuals.append(residual_img)
    ...
    def plot_progressive_residuals(self, original_image, residual_analysis):
        """
        Plot the 3-row progressive residual quantization visualization
        """
        num_scales = len(residual_analysis['residuals'])
        ...
        # Row 1: Residuals (what's left to quantize)
        for i, residual in enumerate(residual_analysis['residuals']):
            img_display = ((residual[0] + 1) / 2).clamp(0, 1)
            img_display = img_display.permute(1, 2, 0).detach().cpu().numpy()
            
            axes[0, i].imshow(img_display)
            pn = self.args['patch_nums'][i]
            if i == 0:
                axes[0, i].set_title(f'Scale {i}\nOriginal Image\n({pn}×{pn} to quantize)', fontsize=12)
            else:
                axes[0, i].set_title(f'Scale {i}\nResidual after {i-1} scales\n({pn}×{pn} to quantize)', fontsize=12)
            axes[0, i].axis('off')
        
        # Row 2: Contributions (what this scale contributed)  
        for i, contribution in enumerate(residual_analysis['contributions']):
            img_display = ((contribution[0] + 1) / 2).clamp(0, 1)
            img_display = img_display.permute(1, 2, 0).detach().cpu().numpy()
            
            axes[1, i].imshow(img_display)
            pn = self.args['patch_nums'][i]
            axes[1, i].set_title(f'Scale {i}\nContribution\n({pn}×{pn} tokens)', fontsize=12)
            axes[1, i].axis('off')
            
        # Row 3: Accumulated (running reconstruction)
        for i, accumulated in enumerate(residual_analysis['accumulated']):
            img_display = ((accumulated[0] + 1) / 2).clamp(0, 1)
            img_display = img_display.permute(1, 2, 0).detach().cpu().numpy()
            
            axes[2, i].imshow(img_display)
            pn = self.args['patch_nums'][i]
            axes[2, i].set_title(f'Scale {i}\nAccumulated\n(Scales 0-{i})', fontsize=12)
            axes[2, i].axis('off')
        
        # Add row labels
        fig.text(0.02, 0.83, 'Residual\n(Left to Quantize)', fontsize=16, fontweight='bold', rotation=90, va='center')
        fig.text(0.02, 0.50, 'Contribution\n(This Scale Added)', fontsize=16, fontweight='bold', rotation=90, va='center')
        fig.text(0.02, 0.17, 'Accumulated\n(Reconstruction So Far)', fontsize=16, fontweight='bold', rotation=90, va='center')
        
        plt.suptitle('VQVAE Progressive Residual Quantization (Algorithm 1)', fontsize=18, fontweight='bold')
        plt.tight_layout()
        plt.subplots_adjust(left=0.08, top=0.93, hspace=0.15, wspace=0.1)
        
        plt.savefig(os.path.join(self.args['save_path'], 'progressive_residuals.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def analyze_token_distribution(self):
        ...
        """
        if not self.args['analyze_distribution']:
            print("Distribution analysis disabled. Set 'analyze_distribution': True to enable.")
            return None
        ...
        print("Analyzing token distribution across limited dataset...")
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # Optionally analyze token distribution across all images
    if vis_args['analyze_distribution']:
        token_stats = visualizer.analyze_token_distribution()
    
    print(f"Analysis complete! Visualizations saved to {vis_args['save_path']}")
    print("Generated:")
    print("- tokens_visualization.png (original + token maps)")
    print("- reconstructions_comparison.png (direct vs VAR reconstructions)")
    print("- progressive_residuals.png (VQVAE Algorithm 1 demonstration)")
    if vis_args['analyze_distribution']:
        print("- codebook_analysis.png (token distribution analysis)")
...
#             pin_memory=True
#         )
#         print(f'[DataLoader] Loaded {len(dataset)} images, bs={batch_size} from {self.args["data_path"]}')
