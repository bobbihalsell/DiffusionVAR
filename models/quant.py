"""
Vector Quantization for VQVAE models.

This module provides the VectorQuantizer2 class used in VQVAE models for
discrete token representation. It includes EMA-based codebook updates and
multi-scale quantization support.
...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        margin = (dist.get_world_size() if dist.initialized() else 1) * (f_BChw.numel() / f_BChw.shape[1]) / self.vocab_size * 0.08
        # margin = pn*pn / 100
