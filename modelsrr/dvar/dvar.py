...
class DiffusionVAR(nn.Module):
    ...
    def infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            x = torch.cat((all_maps), dim=1)  # 2B, L, C
            scl_attn_bias = self.attn_bias_for_masking[:, :, :ed, :ed].to(dtype=x.dtype)
            ...
            for step in range(num_scale_steps):  # ensure s is not negative
                ...
                x_out = self.backbone(x_BLC=x, cond_BD=tcond_BD, attn_bias=scl_attn_bias)
