...
from utils.arg_util import DictObj
torch.serialization.add_safe_globals([DictObj])


os_system = functools.partial(subprocess.call, shell=True)

def echo(info):
    os_system(f'echo "[$(date "+%d-%m-%H:%M:%S")] ({os.path.basename(sys._getframe().f_back.f_code.co_filename)}, line{sys._getframe().f_back.f_lineno})=> {info}"')
...
def time_str(fmt='[%d-%m %H:%M:%S]'):
    return datetime.datetime.now(tz=pytz.timezone('Europe/London')).strftime(fmt)
...
def _change_builtin_print(is_master):
    """Replace builtin print with custom logging."""
...
class SyncPrint(object):
    """Synchronized print that writes to both terminal and file."""
...
class DistLogger(object):
    """Distributed logger that only logs on master rank."""
...
class TensorboardLogger(object):
    """Tensorboard logger with step management."""
...
# def auto_resume(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict, dict]: # TODO 
#     """Automatically resume training from latest checkpoint."""
#     info = []
    
#     # check in the output directory specified in args
#     if hasattr(args, 'output_dir_name') and args.output_dir_name:
#         # Use the output directory directly from args
...
#             # Print detailed checkpoint search info
...
#             print(f"File: {os.path.basename(all_ckpt[0])}")
#             print(f"Resuming from: Epoch {ep+1}, Iteration {it}")
#             print(f"{'='*60}\n")
            
#             info.append(f'[auto_resume success] resume from ep{ep+1}, it{it}')
#             return info, ep, it, ckpt['trainer'], ckpt['args']
    
#     # fallback to old behavior if output_dir_name not available
#     file = os.path.join(args.local_out_dir_path, pattern)
#     all_ckpt = glob_with_latest_modified_first(file)
#     if len(all_ckpt) == 0:
#         info.append(f'[auto_resume] no ckpt found @ {file}')
#         print(f"NO CHECKPOINT FOUND (FALLBACK)")
#         print(f"{'='*60}")
#         print(f"Searched in: {args.local_out_dir_path}/")
#         print(f"Full path: {file}")
#         print(f"Starting fresh training from epoch 1")
#         print(f"{'='*60}\n")
#         info.append(f'[auto_resume quit]')
...
#         print(f"CHECKPOINT LOADED SUCCESSFULLY (FALLBACK)")
#         print(f"{'='*60}")
#         print(f"Directory: {args.local_out_dir_path}/")
#         print(f"File: {os.path.basename(all_ckpt[0])}")
#         print(f"Resuming from: Epoch {ep+1}, Iteration {it}")
#         print(f"{'='*60}\n")
