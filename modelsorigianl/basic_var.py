"""
Basic Building Blocks for Visual Autoregressive (VAR) Models

This module provides the fundamental building blocks used in Visual Autoregressive
(VAR) models, including feed-forward networks, adaptive layer normalization,
and self-attention mechanisms. These components are designed for efficient
progressive image generation through next-scale prediction.

The implementation includes optimized versions using Flash Attention and other
fused operations when available, providing significant speedup for large-scale
...
class FFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., fused_if_available=True):
        ...
        :param fused_if_available: Whether to use fused operations when available
        """
        super().__init__()
    ...
    def forward(self, x):
        """
        :param x: Input tensor of shape (B, L, C) where B=batch, L=sequence, C=features
        :return: Output tensor of shape (B, L, C) after feed-forward processing
        """
...
class SelfAttention(nn.Module):
    """
    Multi-head self-attention mechanism with optional optimizations.
    """
    ...
    def __init__(
        ...
        """
        :param block_idx: Index of the transformer block (for debugging)
        :param embed_dim: Embedding dimension (must be divisible by num_heads)
        :param num_heads: Number of attention heads
        :param attn_drop: Attention dropout probability
        :param proj_drop: Projection dropout probability
        :param attn_l2_norm: Whether to use L2 normalization in attention
        :param flash_if_available: Whether to use Flash Attention when available
        """
        super().__init__()
    ...
    def kv_caching(self, enable: bool):
        """
        :param enable: Whether to enable key-value caching
        """
    ...
    def forward(self, x, attn_bias):
        """
        :param x: Input tensor of shape (B, L, C) where B=batch, L=sequence, C=features
        :param attn_bias: Optional attention bias tensor (None during inference with caching)
        :return: Output tensor of shape (B, L, C) after self-attention processing
        """
        B, L, C = x.shape
...
class AdaLNSelfAttn(nn.Module):
    ...
    def __init__(
        ...
        """
        :param block_idx: Index of the transformer block
        :param last_drop_p: Last dropout probability
        :param embed_dim: Embedding dimension
        :param cond_dim: Conditioning dimension
        :param shared_aln: Whether to use shared adaptive layer normalization
        :param norm_layer: Normalization layer class
        :param num_heads: Number of attention heads
        :param mlp_ratio: MLP expansion ratio
        :param drop: Dropout probability
        :param attn_drop: Attention dropout probability
        :param drop_path: Drop path probability
        :param attn_l2_norm: Whether to use L2 normalization in attention
        :param flash_if_available: Whether to use Flash Attention when available
        :param fused_if_available: Whether to use fused operations when available
        """
    ...
    def forward(self, x, cond_BD, attn_bias):
        """
        :param x: Input tensor of shape (B, L, C) where B=batch, L=sequence, C=embed_dim
        :param cond_BD: Conditioning tensor of shape (B, D) where D=cond_dim
        :param attn_bias: Optional attention bias tensor (None during inference with caching)
        :return: Output tensor of shape (B, L, C) after processing
        """
        if self.shared_aln:
...
class AdaLNBeforeHead(nn.Module):
    """
    Adaptive layer normalization before the final head.
    """
    
    def __init__(self, C, D, norm_layer):
        """
        :param C: Embedding dimension
        :param D: Conditioning dimension
        :param norm_layer: Normalization layer class
        """
        super().__init__()
        self.C, self.D = C, D
    ...
    def forward(self, x_BLC: torch.Tensor, cond_BD: torch.Tensor):
        """
        :param x_BLC: Input tensor of shape (B, L, C) where B=batch, L=sequence, C=embed_dim
        :param cond_BD: Conditioning tensor of shape (B, D) where D=cond_dim
        :return: Normalized and scaled tensor of shape (B, L, C)
        """
